...
- Also exposes SQ (= mean TP IoU), RQ, and the pair-score distributions
  (IoU>0 pairs only, as in the official plot function).

Implementation note: IoU matrix via pycocotools C implementation; Dice derived by
the exact identity dice = 2*iou/(1+iou) for iou>0 else 0.
"""
...
class PQResult:
    pq: float
    sq: float                 # mean IoU over TP pairs
    rq: float                 # TP / (TP + 0.5FP + 0.5FN)
    tp: int
    fp: int
    fn: int
    n_gt_images: int
...
def evaluate_from_rle_frames(gt_frames: list[tuple[str, list[str]]],
                             pred_frames: dict[str, list[str]],
                             height: int = 2048, width: int = 2048) -> PQResult:
    """Convenience wrapper over compressed-RLE strings (submission format)."""
    from .rle import rle_to_mask
    gt_rows = [(stem, [rle_to_mask(r, height, width) for r in rles]) for stem, rles in gt_frames]
    preds = {stem: [rle_to_mask(r, height, width) for r in rles]
             for stem, rles in pred_frames.items()}
    return evaluate_pq(gt_rows, preds)
