...
class Instance:
    ann_id: str
    image_key: str          # annotator-image id, e.g. "040301-20140609195854Bh"
    stem: str               # physical jpeg stem, e.g. "20140609195854Bh"
    category_id: int
    area: float
...
def load_annotations(json_path: Path = TRAIN_JSON) -> dict[str, AnnImage]:
    ...
    for a in d["annotations"]:
        seg = a["segmentation"]
        # segmentation is stored as a JSON string of a list-of-polygons in this file
        ...
        inst = Instance(
            ann_id=a["id"],
            image_key=a["image_id"],
            stem=stem_of[a["image_id"]],
